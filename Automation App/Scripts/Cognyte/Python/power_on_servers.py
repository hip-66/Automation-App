# -*- coding: utf-8 -*-
"""
Power ON one or more Dell servers by iDRAC IP, using
...
def read_targets():
    ...
    if not servers and sys.stdin is not None and sys.stdin.isatty():
        ip = input("Enter iDRAC IP to power on: ").strip()
        if ip:
            servers = [ip]
    return servers
...
def is_reachable(ip, timeout_ms=2000, attempts=2):
    """Fast ICMP pre-check on the iDRAC IP: an unreachable/dead iDRAC is
    detected in ~2-4 seconds and SKIPPED, instead of letting racadm stall on it
    for its full 120s timeout - so one dead iDRAC never holds up the rest of the
    list. (The iDRAC answers on standby power even while the server itself is
    off, which is exactly the state we power ON from.) Checks for 'ttl=' in the
    reply because Windows ping exits 0 even on a 'Destination host unreachable'
    answer from a router - only a real reply from the target itself has a TTL."""
...
def main():
    print("=== Power On Servers (racadm serveraction powerup) ===", flush=True)
    ...
    for ip in servers:
        ...
        print(f"Powering on {ip} ...", flush=True)
        try:
            proc = subprocess.run(
                [racadm, "-r", ip, "-u", IDRAC_USER, "-p", IDRAC_PASS,
                 "--nocertwarn", "serveraction", "powerup"],
                capture_output=True, text=True, timeout=120
            )
            for stream in (proc.stdout, proc.stderr):
                if stream and stream.strip():
                    print("  " + stream.strip().replace("\n", "\n  "), flush=True)
            # racadm returns a non-zero code when the server is ALREADY on
            # ("Server is already powered ON") - treat that as success, not a
            # failure, since the desired end state (powered on) is met.
            combined = ((proc.stdout or "") + (proc.stderr or "")).lower()
            already_on = "already powered on" in combined or "already on" in combined
            if proc.returncode == 0 or already_on:
                if already_on:
                    print(f"OK: {ip} is already powered on.", flush=True)
                else:
                    print(f"OK: power on command sent to {ip}.", flush=True)
                ok += 1
            else:
