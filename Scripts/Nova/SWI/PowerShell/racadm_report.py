...
# ==========================================================
# Auto install required Python libraries if they are missing
# ==========================================================
def ensure_python_package(import_name, pip_name=None):
    """
    Checks if a Python package exists.
    If missing, installs it automatically using the same Python executable.
    ...
    if importlib.util.find_spec(import_name) is not None:
        print(f"[OK] Python library '{pip_name}' already installed")
        return True

    print(f"[MISSING] Python library '{pip_name}' not found. Installing now...")
    ...
    try:
        ...
        print(f"[OK] Python library '{pip_name}' installed successfully")
        return True
    except subprocess.CalledProcessError as e:
        print(f"[ERROR] Failed to install Python library '{pip_name}'. Error: {e}")
        print("[TIP] Try running this manually:")
        print(f"      {sys.executable} -m pip install {pip_name}")
        return False

# Add every external Python dependency here
REQUIRED_PACKAGES = [
    ("docx", "python-docx"),
]
